...
class Solution:
    def solve(self, n: int, m: int, k: int, map_info: list[list[int]]) -> int:
        ...
        q: deque[tuple[int, int, int, int, int]] = deque()
        q.append((0, 0, 0, True, 1))
        for i in range(k + 1):
            visited[i][0][0] = 1

        while q:
            y, x, break_cnt, is_day, l = q.popleft()
            if y == n - 1 and x == m - 1:
                ans = l
                break
            ...
            for i in range(4):
                ...
                if map_info[next_y][next_x] == 0:
                    q.append((next_y, next_x, break_cnt, not is_day, l + 1))
                    visited[break_cnt][next_y][next_x] = 1

                if map_info[next_y][next_x] == 1 and break_cnt < k and is_day:
                    if visited[break_cnt + 1][next_y][next_x] == 0:
                        q.append((next_y, next_x, break_cnt + 1, not is_day, l + 1))
                        visited[break_cnt + 1][next_y][next_x] = 1

                if not is_day:
                    q.append((y, x, break_cnt, not is_day, l+1))
